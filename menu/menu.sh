...
export COLOR1="$(cat /etc/phantomshield/theme/$colornow | grep -w "TEXT" | cut -d: -f2|sed 's/ //g')"
export COLBG1="$(cat /etc/phantomshield/theme/$colornow | grep -w "BG" | cut -d: -f2|sed 's/ //g')"                    
###########- END COLOR CODE -##########
...
Name=$(curl -sS https://raw.githubusercontent.com/Z0nure/ipmini/main/auth | grep $MYIP | awk '{print $2}')
Isadmin=$(curl -sS https://raw.githubusercontent.com/Z0nure/ipmini/main/auth | grep $MYIP | awk '{print $5}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/Z0nure/ipmini/main/auth | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/Z0nure/ipmini/main/auth | grep $MYIP | awk '{print $3}')
fi
export RED='\033[0;31m'
...
echo "IP=$host" > /var/lib/phantom-pro/ipvps.conf
echo ""
...
sleep 2
wget -q -O /root/alpha3-update "https://raw.githubusercontent.com/Z0nure/update/main/alpha3-update" && chmod +x /root/alpha3-update
sleep 2
./alpha3-update
sleep 5
rm /root/alpha3-update
rm /opt/.ver
version_up=$(curl -sS https://raw.githubusercontent.com/Z0nure/access/main/.ver)
echo "$version_up" > /opt/.ver
echo -e "$COLOR1│${NC}  $COLOR1[INFO]${NC} Successfully Up To Date!"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
echo -e "$COLOR1│${NC}              • PHANTOM SHIELD •                $COLOR1│$NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo ""
read -n 1 -s -r -p "  Press any key to back on menu"
menu
...
serverV=$(curl -sS https://raw.githubusercontent.com/Z0nure/access/main/.ver)
if [ "$Isadmin" = "ON" ]; then

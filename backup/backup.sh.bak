...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/Z0nure/access/main/auth > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/Z0nure/access/main/auth | grep $MYIP | awk '{print $2}')
...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/Z0nure/access/main/auth | grep $MYIP | awk '{print $2}')
...
cp -r /var/lib/phantomshield-pro/ /root/backup/phantomshield-pro &> /dev/null
...
LLatest=`date`
Get_Data () {
git clone https://github.com/Z0nure/backup.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    DATE=$(date +'%d %B %Y')
    cd /root/user-backup
    git config --global user.email "rensyo90@gmail.com" &> /dev/null
    git config --global user.name "Z0nure" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    ...
    git push -f https://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@github.com/Z0nure/backup.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/Z0nure/backup/main/$NameUser/$NameUser.zip"
sleep 1
